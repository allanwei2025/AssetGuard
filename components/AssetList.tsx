...
const AssetList: React.FC<AssetListProps> = ({ assets, onAssetClick, onDelete }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-t-3xl shadow-top border border-slate-200 mt-2">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-32">
        ...
        {filteredAssets.map(asset => (
          <div 
            ...
            className={`p-3 rounded-xl border flex justify-between items-center cursor-pointer transition-colors active:scale-[0.98] ${
          ...
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className={`font-semibold truncate text-sm ${asset.status === AssetStatus.PENDING ? 'text-slate-800' : 'text-slate-600'}`}>{asset.name}</h3>
                {asset.status === AssetStatus.FOUND && (
                  <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full font-bold border border-green-200 whitespace-nowrap">已盘点</span>
                )}
                 {asset.status === AssetStatus.EXTRA && (
                  <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full font-bold border border-blue-200 whitespace-nowrap">新增</span>
                )}
              </div>
            ...
            <div className="ml-3 flex flex-col items-end gap-2">
                ...
                {/* Delete button for EXTRA assets */}
                {asset.status === AssetStatus.EXTRA && (
                  <button 
